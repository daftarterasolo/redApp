function tempAlert(duration) {
  ...
  function detailItem(arr, jenisTera, iter, penera) {
    ...
    if (jenisTera === "tera") {
      ...
      detailStr += `<tbody><tr><td>${arr[2]}</td><td>${arr[3]}</td><td>${arr[6]} / ${arr[7]} / ${arr[8]}</td><td>${arr[9]}</td><td>${arr[11]}${arr[10]} - ${arr[11]}${arr[10]+arr[13]-1}</td><td>${arr[12]}</td><td>${arr[13]}</td><td>${arr[14]}</td><td>${arr[15] === ""? `${strPenera} & ${strPenera2}` : arr[15]}</td><td><input type="button" class="sertifikatBtn" name="${arr[1]}-${iter}-${jenisTera}" id="${arr[1]}-${iter}-${jenisTera}" value = "Sertifikat"></td></tr></tbody>`;    
